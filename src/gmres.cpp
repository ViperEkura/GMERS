...
void SparseMatrixv(const uint* rowPtr, const uint* colInd, const double* values,
...
RESULT gmres(SparseMatrix<double> *A_d, double *x_d, double *_b)
{
    ...
    {
        // ==========外迭代============
        SparseMatrixv(A_d->rows, A_d->cols, A_d->vals, x_d, r0.data(), N);  // 不可修改此步操作中相关数据的存储精度和SparseMatrixV计算精度
        ...
        do
        {
            ...
            SparseMatrixv(A_d->rows, A_d->cols, A_d->vals, V_i.data(), r0.data(), N);
...
void initialize(SparseMatrix<double> *A, double *x, double *b)
{
    ...
    SparseMatrixv(A->rows, A->cols, A->vals, x, b, N); 
